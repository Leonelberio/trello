...
const baseUrl = "https://trello-tczf.onrender.com/board";
...
export const createBoard = async (props, dispatch) => {
  ...
  if (!(props.title && props.backgroundImageLink)) {
    ...
    dispatch(
      openAlert({
        message: "Veuillez saisir un titre pour le projet!",
        severity: "warning",
      })
    );
    return;
  ...
  try {
    ...
    dispatch(
      openAlert({
        message: `${res.data.title} le projet a été créé avec succès`,
        severity: "success",
      })
    );
  } catch (error) {
